...
if (!function_exists('view')) {
    ...
    function view(string $name, array $data) {
        ...
        $viewFile = $viewsPath . $name . '.php';
        
        // Check if the view file exists
        if (!file_exists($viewFile)) {
            throw new Exception("View file '{$name}' not found in {$viewsPath}");
            return false;
        }
        ...
        try {
            ...
            ob_end_clean();
            throw new Exception("Error loading view: " . $e->getMessage());
            return false;
        }
    }

    /**
 * Load a view file and extract data variables
 * 
 ...
 * @return string|bool Returns rendered view content or false on failure
 */
}
...
if (!function_exists('validate')) {
    ...
    function validate(array $data, array $rules) {
        ...
        foreach ($rules as $field => $rule) {
            ...
            if (strpos($rule, 'postal_code') !== false) {
                if (!preg_match('/^\d{4,6}$/', $value)) {
                    $errors[$field] = "The postal code must be 4-6 digits.";
                }
            }
        }
